    ...
    def loss(y_true, y_pred):
        ...
        for c_batch in range(batch_size):
            ...
            for i in range(tot_loc):
                ...
                diff = d[:-2] - d[1:-1]
                # tf.print(diff[0])
                # tf.print(F"NEW LOCATION")
                # # for j in range(max_depth_idx[i_loc]):
                # for j in range(1):
                #     tf.print(F"%%%%%%%%%%%%% {j} %%%%%%%%%%%%%%%%%%%%%%")
                ...
                # tf.print(F"{i} Temperature: {t}")
                # tf.print(F"{i} Salinity: {s}")
                # tf.print(F"{i} Density: {d}")
                # tf.print(F"{i} Error: {error_mon}")

        y_true_f = K.flatten(y_true)
        y_pred_f = K.flatten(y_pred)
        # tf.print(F"1 Shape: {temp.shape}")
        # tf.print(F"Type: {temp.dtype}")
        # tf.print(F"2 Shape: {error_mon.shape}")
        # tf.print(F"Type: {error_mon.dtype}")
        # return error_mon
        deltemp = tf.math.reduce_mean(tf.math.squared_difference(y_true_f, y_pred_f))
        tf.print(F"{i} RMSE: {deltemp}")
        tf.print(F"{i} Error: {error_mon}")
        # return deltemp  + error_mon
        # return tf.math.reduce_mean(tf.math.squared_difference(y_true_f, y_pred_f))
        return tf.math.reduce_mean(tf.math.squared_difference(y_true_f, y_pred_f)) + error_mon
